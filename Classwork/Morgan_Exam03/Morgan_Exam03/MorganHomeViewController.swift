...
let allGames: [Game] = [
    badminton, basketball, tableTennis, volleyball, chess, snooker, swimming, boxing, karate, handball, soccer, cricket, tennis, hockey, golf, trackandField, cycling, rugby, surfing
    ].sorted { ($0.name ?? "") < ($1.name ?? "") }

class MorganHomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // Create Cell
        let cell = MorganTVOL.dequeueReusableCell(withIdentifier: "MorganCell", for: indexPath)
        
        // Populate Cell
        cell.textLabel?.text = allGames[indexPath.row].name
    ...
    @IBOutlet weak var MorganTVOL: UITableView!
    ...
    override func viewDidLoad() {
        ...
        MorganTVOL.delegate = self
        MorganTVOL.dataSource = self
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if transition == "MorganDescriptionSegue" {
            let destination = segue.destination as! MorganGameController
            
            if let indexPath = MorganTVOL.indexPathForSelectedRow {
                destination.selectedGame = allGames[indexPath.row]
